...
export function JourneyAskedQuestions({ journeyId, onClose }: Props) {
  ...
  const getDefaultValues = async (): Promise<AskedQuestionsSchema> => {
    ...
    const askedQuestions = await getAskedQuestions(journeyId)

    if (askedQuestions.length === 0) {
      askedQuestionsDefaultValues.askedQuestions[0].journeyId = journeyId
      return {
        ...askedQuestionsDefaultValues,
        title: journey.title,
      }
    ...
    const newAskedQuestions = askedQuestions.map(
      (askedQuestion): AskedQuestionSchema => {
        return {
          ...
          journeyId: askedQuestion.journeyId,
        }
      },
    )
  ...
  const handleChange = async (data: AskedQuestionsSchema) => {
    try {
      ...
      toast.success(
        t('journey.asked-question.saved-message', {
  ...
  return (
    <div className='flex flex-col gap-6'>
      <div className='border-b-chinese-white z-overlay sticky top-0 flex flex-col justify-between gap-4 border-b bg-white py-2 md:flex-row'>
        ...
        >
          {t('journey.asked-question.save-label')}
        </Button>
      </div>
      <div className='flex flex-col items-start gap-6'>
        ...
        <Button variant='action' onClick={handleAdd}>
          {t('journey.asked-question.add-question-label')}
        </Button>
      </div>
    </div>
  )
      ...
      if (askedQuestionId) {
        startTransition(async () => {
          try {
            ...
            toast.success(
              t('journey.asked-question.deleted-question-message', {
                title: title[locale],
              }),
            )
            fields.remove(index)
        ...
        <div className='flex items-center justify-between gap-4'>
          <div className='flex items-center gap-2'>
            ...
            <div className='bg-anti-flash-white border-chinese-white rounded-sm border px-2 py-1 text-xs leading-4 font-bold uppercase'>
              {t('journey.asked-question.question-number', {
                number: index + 1,
              })}
            </div>
          </div>
